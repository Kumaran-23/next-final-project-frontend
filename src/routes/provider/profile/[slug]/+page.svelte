<script>
    ...
    async function updateProfile(evt) {
      ...
      const resp = await fetch(PUBLIC_BACKEND_BASE_URL + '/providers/update', {
        method: 'PATCH',
        mode: 'cors',
        headers: {
      ...
      if (resp.status == 200) {
        ...
        clicked = false;
        formErrors = responseData.formErrors;
    ...
    <div class="flex flex-col px-8 py-20 mx-auto md:h-screen">
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-l xl:p-0 dark:bg-gray-800 dark:border-gray-700">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" on:submit={updateProfile}>
            <div>
              <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
              <input value={data.provider.name} type="text" name="name" id="name" placeholder="John Doe" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
            ...
            <div>
              ...
              <input value={data.provider.description} type="text" name="description" id="description" placeholder="Tell us about yourself!" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
            </div>
            ...
            <div class="form-control w-full">
              ...
              {:else}
              <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 w-full">Save</button>
              {/if}
            </div>
          </form>
        </div>
